...
# ✅ Translate via Local CSV Lookup
...
# ✅ Translate via Gemini (if not found in CSV)
...
def bot_translate_to_kaithi():
    print("🔤 Welcome to the English ➡️ Kaithi Translator Bot!")
    while True:
        user_input = input("🔍 Enter an English word (or type 'exit' to quit): ").strip()
        ...
        if kaithi_translation is None:
            print("📁 Word not found in CSV. Asking Gemini for translation...")
            kaithi_translation = translate_with_gemini(user_input)

        print(f"📝 Kaithi translation for '{user_input}': {kaithi_translation}")
